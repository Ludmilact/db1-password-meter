...
public class ResultadoDeAnaliseRequerimentos extends ResultadoDeAnalise {
    ...
    private void calcularRequerimentos(String senha){
        ...
        if (ValidaRequisitos.obterQuantidadeDeLetrasMinusculas(senha) > 0)
            contagem += 1;
        if (ValidaRequisitos.obterQuantidadeDeLetrasMaisculas(senha) > 0)
            contagem += 1;
        if (ValidaRequisitos.obterQuantidadeDeNumeros(senha) > 0)
            contagem += 1;
        if (ValidaRequisitos.obterQuantidadeDeSimbolos(senha) > 0)
            contagem += 1;
